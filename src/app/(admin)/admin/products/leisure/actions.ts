...
// Fetch leisure collection products for table
export async function fetchLeisureCollectionTable(): Promise<FetchLeisureCollectionResult> {
  try {
    ...
    console.error("Error fetching leisure collection for table:", error);
    return {
      success: false,
      error:
...
export async function toggleProductPublish(
  ...
  try {
    ...
    revalidatePath("/admin/products/leisure");
